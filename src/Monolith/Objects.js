...
function getTerrainHeignt(posx, posz, considerWater = true) {
    ...
    if (terrainRandoms[4]) val += noise.noise2D(x / 450, z / 800) * noise.noise2D(x / 30, z / 30) ** 3 * terrainVal[2]
...
export function alienTerrain() {
    ...
    if (alienType === 'rocks') {
        ...
        for (let i = 0; i < geo.attributes.position.count; i++) {
            ...
            geo.attributes.position.setX(i, x * (1 + noise.noise2D(y / xzScale, z / xzScale) / 5))
            geo.attributes.position.setX(i, z * (1 + noise.noise2D(x / xzScale, y / xzScale) / 5))
            geo.attributes.position.setY(i, y * (.5 + noise.noise2D(x / yscale, z / yscale) * .5) * (isFloating ? 1 : 4))
        }
